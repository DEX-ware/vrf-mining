...
Miller et al.~\cite{miller2015nonoutsourceable} formalises cryptocurrency mining as \emph{scratch-off puzzles}, defines \emph{non-outsourceable scratch-off puzzles}, and proposes two constructions.
One of these two constructions achieves \emph{weak non-outsourceability} (i.e., miners can steal the mining reward), and the other achieves \emph{strong non-outsourceability} (i.e., miners can anonymously steal the mining reward).
...
To achieve \emph{strong non-outsourceability} (i.e., make the stealing behaviours anonymous), the strong non-outsourceable scratch-off puzzle replaces the plaintext nonce in the block with a Zero Knowledge Proof proving the statement ``I know a valid nonce''.
