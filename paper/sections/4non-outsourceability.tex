...
The anonymity of malicious miners defined in~\cite{} is equivalent to \textbf{Transaction Unlinkability}~\cite{}: given two arbitrary transactions, distinguishing whether they have the same output address requires knowing the private keys holding their outputs.
...
Similar with the construction achieving \textbf{Strong Non-outsourceability} in~\cite{}, VRF-based mining achieves \textbf{Transaction Unlinkability} by allowing a malicious miner to use a freshly generated address to steal cryptocurrency.
...
Then the malicious miner can then spend his stolen cryptocurrency anonymously using numerous techniques, such as mixing services~\cite{}, stealth addresses~\cite{}, and Zero Knowledge Contingent Payments~\cite{}.
