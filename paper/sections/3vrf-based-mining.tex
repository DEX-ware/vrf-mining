...
\begin{enumerate}
    ...
    \item ...
    \item Miner then submit the $pk$, $\beta$, and the proof $\pi$ to the node;
    \item The node then check against $\mathsf{VRFVerify}(pk, \alpha, \beta, \pi)$, and whether the \texttt{scriptPubKey} in the coinbase transaction corresponds to $pk$.
    If both results are valid, a new block is mined, and the coinbase reward is paid to the miner.
